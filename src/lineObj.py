...
def qcqp_approximation(N, M, Q, c, A, all_Y, all_Z, C, b, u, epslong):
    ...
    model = gp.Model()
    var_value = [1.8294890001986117, 0.7454090690796642, -0.25042858766783593, 0.15060807062925227, -0.0012409492512921994, 0.24733440037215057, -0.5599177037791294, 0.913048825686068, 0.5910477229072426, 0.007790297885252542, -0.020745236181805602, 0.5745707262782478, -0.4733290453143005, -0.21162212772609218, -0.44069460651179426, -0.09358907328603334, 0.10793036512406814, 0.42199063051601005, -0.9620736837009572, 1.1754947879119078, -89.28933592175983]
    # Variables
    vars = []
    for i in range(N-1): 
        # -GRB.INFINITY
        vars.append(model.addVar(lb=var_value[i], ub=var_value[i], vtype=GRB.CONTINUOUS))

    vars.append(model.addVar(lb=var_value[-1], ub=var_value[-1], vtype=GRB.CONTINUOUS))
    # obj = gp.QuadExpr()
    # for i in range(N):
    #     for j in range(N):
    #         if Q[i][j] != 0:
    ...
    for m in range(M):
        ...
        for index in range(layers_count[0]):
            ...
            # model.addConstr(aTv - z_i[2*index] <= M_bar*temp_var2)
            # model.addConstr(w_i[2*index] <= M_bar*(1-temp_var2))
            ...
            # model.addConstr(bTv - z_i[2*index+1] <= M_bar*temp_var2)
            # model.addConstr(w_i[2*index+1] <= M_bar*(1-temp_var2))
