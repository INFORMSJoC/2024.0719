...
def qcqp_approximation(N, M, Q, c, A, all_Y, all_Z, C, b, u, epslong):
    ...
    t = []
    for i in range(M):
        t.append(model.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS))
    ...
    for m in range(M):
        ...
        for i in range(layers_count[0]):
            if 2*i+1 < k_upper:
                ...
                tempConstr = model.addConstr(xi_0 >= -temp_y1)
                model.update()
                tempConstr.Lazy = 2
                tempConstr = model.addConstr(xi_0 >= temp_y1)
                model.update()
                tempConstr.Lazy = 2
                tempConstr = model.addConstr(eta_0 >= -temp_y2)
                model.update()
                tempConstr.Lazy = 2
                tempConstr = model.addConstr(eta_0 >= temp_y2)
                model.update()
                ...
                for index_mul in range(0, 2**multip_number+1):
                    tempConstr = model.addConstr( math.cos(math.pi*index_mul / (2**(multip_number+1))) * xi_0 + math.sin(math.pi*index_mul / (2**(multip_number+1)))*eta_0<= temp_y3)
                    model.update()
                    tempConstr.Lazy = 2
            else:
        ...
        # Z部分
        if k_low == 1:
            tempConstr = model.addConstr( z_i[0] >= t[m] )
            model.update()
            tempConstr.Lazy = 2
            continue
        ...
        for i in range(layers_count[0]):
            ...
            # j 就是index_mul  v是multip_number
            for index_mul in range(0, 2**multip_number+1):
                tempConstr = model.addConstr( math.cos(math.pi*index_mul / (2**(multip_number+1))) * temp_y1 + math.sin(math.pi*index_mul / (2**(multip_number+1)))*temp_y2<= temp_y3)
                model.update()
                tempConstr.Lazy = 2
        ...
        for index in range(layers_count[0]):
            ...
            tempConstr = model.addConstr(aTv >= -z_i[2*index])
            model.update()
            tempConstr.Lazy = 2
            tempConstr = model.addConstr(aTv >= z_i[2*index])
            model.update()
            ...
            tempConstr = model.addConstr(bTv >= -z_i[2*index+1])
            model.update()
            tempConstr.Lazy = 2
            tempConstr = model.addConstr(bTv >= z_i[2*index+1])
            model.update()
                ...
                # 47
                    ...
                    if (index+1)//2 % 2 != 0: # 奇数 1 5
                        ...
                        for temp_i in range(2**multip_number+1):
                            cTv += bT[temp_i]*v[layer_index+1][(index-1)//2][temp_i]
                # KKT 以及 本来的约束
                tempConstr = model.addConstr(eTv+cTv >= 0)
                model.update()
                tempConstr.Lazy = 2
        ...
        tempConstr = model.addConstr(min_eTv - t[m]>=0)
        model.update()
        tempConstr.Lazy = 2
