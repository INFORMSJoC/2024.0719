...
# TODO: ADMM check the feasible solution: lower bound vs upper bound 
...
def qcqp_optimize(N, M, A, all_Y, all_Z, C, b, u, epslong):
    ...
    vars = model.addMVar(shape=N, lb=-2*N, ub=2*N, vtype=GRB.CONTINUOUS)  

    # Objective version 1
    model.setObjective( (vars @ np.array(A[0]) @ vars) + (2*np.array(C[0]) @ vars), GRB.MINIMIZE)   
    ...
    for m in range(1,M):  
        ...
        t = model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS)
        ...
        n_cons_introded_by_y = yP.shape[0] 
        for i in range(n_cons_introded_by_y):
            tp_lp_exp = gp.LinExpr()
            for j in range(yP.shape[1]):
                tp_lp_exp = tp_lp_exp + yP[i,j] * y_i[j] 
            for j in range(yQ.shape[1]):
                tp_lp_exp = tp_lp_exp + yQ[i,j] * y_extra_vars[j] 
            model.addConstr( tp_lp_exp + t*yp[i] >= 0)
        ...
        for i in range(len(z_i)):
            tmp_lin_expr = gp.LinExpr()
            for j in range(zP.shape[0]):
                tmp_lin_expr = tmp_lin_expr + zP[j,i] * extra_v[j] 
            model.addConstr( tmp_lin_expr + z_i[i] ==0)
        model.addConstr(zQ.T @ extra_v == 0)
        for i in range(zP.shape[0]):
            model.addConstr(extra_v[i] - extra_bin[i] * 9999 <= 0)
            model.addConstr(zP[i,:] @ extra_w + zQ[i,:] @ extra_vars + zp[i] - (1-extra_bin[i]) * 9999 <= 0)
