...
def qcqp_approximation(N, M, obj_Q, obj_c, A, all_Y, all_Z, C, b, u, epslong):
    ...
    for m in range(M):
        ...
        # for i in range(k_low):
        #     temp += z_i[i] * z_i[i]
        # model.addConstr(temp- , GRB.GREATER_EQUAL, 0)


        if k_low == 1:
            model.addLConstr(z_i[0]- t[m], GRB.GREATER_EQUAL, 0)
            # model.addLConstr(z_i[0]+ , GRB.GREATER_EQUAL, 0)
            continue
        ...
        # v_equ = [model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS) for i in range(v_number)]
        # total_expr_equ = [model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS) for i in range(v_number)]
        for i in range(v_number):
            ...
            # model.addConstr(v[i] == v_equ[i])
            # model.addConstr(total_expr[i] == total_expr_equ[i])
            # model.addSOS(GRB.SOS_TYPE1, [v_equ[i], total_expr_equ[i]])
    ...
    # model.setParam('IntFeasTol', 0.0000001)
    try:
        model.optimize()
