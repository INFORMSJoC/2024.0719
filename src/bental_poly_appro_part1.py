...
def qcqp_optimize(N, M, A, all_Y, all_Z, C, b, u, epslong):
    ...
    for m in range(1,M):  
        ...
        # # yp = sparse.csr_matrix(yp) 
        # # y_i_vars = model.addMVar(shape=k_upper, vtype=GRB.CONTINUOUS)
        # # for y_i_idx in range(k_upper):
        # #     model.addConstr(y_i_vars[y_i_idx] == y_i[y_i_idx])
        ...
        #         tp_lp_exp += yP[i,j] * y_i[j] 
        #     model.addConstr( tp_lp_exp + yQ[i,:] @ y_extra_vars + t[0]*yp[i] >= 0)
